...
internal static class LeagueClientLinqQueries
{
    ...
    {
        return paths.GroupBy(p => p.Key.Count(c => c == '/') == 1 ? null : p.Key.SplitAndRemoveEmptyEntries('/')[0]);
    }

    /// <summary>
    ///     Groups events by module.
    /// </summary>
    public static IEnumerable<IGrouping<string, KeyValuePair<string, string>>> GroupByModule(
        this IEnumerable<KeyValuePair<string, string>> events)
    {
...
{
    public static string EventToPath(this string @event)
    {
        return @event.Replace("OnJsonApiEvent", "").Replace("_", "/");
    }

    public static bool EventEqualsPath(this string @event, string path)
    {
        return @event.EventToPath() == path.Trim('\"');
    }
}