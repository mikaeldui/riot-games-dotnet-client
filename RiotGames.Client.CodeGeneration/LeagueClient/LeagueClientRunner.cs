...
internal static class LeagueClientRunner
{
    public static async Task GenerateCodeAsync()
    {
        ...
        var schema = await MingweiSamuelSchemasClient.GetLcuOpenApiSchemaAsync();

        _console(
            $"League Client: Downloaded spec file containing {schema.Paths?.Count} paths and {schema.Components?.Schemas?.Count} component schemas.");
        ...
        _console(
            $"Downloaded help.json containing {help.Events.Count} events, {help.Types.Count} types and {help.Functions.Count} functions.");
        ...
        Dictionary<string, string> events)
    {
        ...
        FileWriter.WriteLeagueClientFile(code);
    }

    private static void _console(string message)
    {
        Console.WriteLine("League Client: " + message);
    }
}