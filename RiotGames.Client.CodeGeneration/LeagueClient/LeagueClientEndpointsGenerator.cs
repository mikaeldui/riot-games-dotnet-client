...
using LcuPathObject =
    OpenApiPathObject<OpenApiMethodObject<LcuParameterObject, LcuSchemaObject>,
...
internal class LeagueClientEndpointsGenerator : OpenApiEndpointGeneratorBase<LcuPathObject, LcuMethodObject,
    LcuMethodObject, LcuMethodObject, LcuParameterObject, LcuSchemaObject>
{
    ...
    private LeagueClientEndpointsGenerator(string className, string[] enums, bool partialClass = true) : base(className,
        partialClass)
    {
        ClassName = className;
        Enums = enums;
    ...
    protected override void AddEndpoint(EndpointDefinition? endpoint)
    {
        ...
        if (endpoint.Identifier.StartsWith("Get") && endpoint.RequestPathParameters != null &&
            endpoint.RequestPathParameters.Count == 1)
        {
            var parameter = endpoint.RequestPathParameters.Single();

            if (LeagueClientHacks.BasicInterfaces.TryGetBasicInterfaceIdentifier("LeagueClient", parameter.Value + "?",
                    parameter.Key.ToPascalCase(), out var interfaceIdentifier))
            {
                var methodIdentifier = endpoint.Identifier;
                var parameterIdentifier = interfaceIdentifier[1..].RemoveEnd("Id");
    ...
    private void _addGroupsAsNestedClassesWithEndpoints(IEnumerable<IGrouping<string?, Path>> groupedPaths,
        IEnumerable<IGrouping<string, KeyValuePair<string, string>>>? groupedEvents, string? className = null)
    {
        foreach (var group in groupedPaths.Where(g => g.Key != null))
        {
            var eventGroup = groupedEvents?.SingleOrDefault(g => g.Key == group.Key);

            var versionSuffix = false;
            ...
            if (versioned.Count() > 1)
            {
                ...
                if (Math.Abs(firstVersionCount - secondVersionCount) >
                    (float) Math.Max(firstVersionCount, secondVersionCount) / 5 * 2)
                {
                ...
                else
                {
                    ...
                    foreach (var versionedModule in versioned)
                    {
                        ...
                        _addGroupsAsNestedClassesWithEndpoints(versionedModule.GroupByModule(),
                            versionedModuleEvents?.SelectMany(e => e)?.GroupByModule(),
            ...
            var moduleGenerator = new LeagueClientModuleGenerator(
                (className ?? group.Key.RemoveChars('{', '}').ToPascalCase())
                .FixGamePrefixes(), Enums, versionSuffix, true);
    ...
    public virtual void AddGroupsAsNestedClassesWithEndpoints(IEnumerable<IGrouping<string?, Path>> groupedPaths,
        IEnumerable<IGrouping<string, KeyValuePair<string, string>>> groupedEvents, string? className = null)
    {
        // Root group
        ...
        {
            var lolGroups =
                groupedPaths.Where(g => g is {Key: not null, Key: not "lol-tft"} && g.Key.StartsWith("lol-"));
            var lolEvents = groupedEvents.Where(g => g.Key.StartsWith("lol-"));
            var generator = new LeagueClientEndpointsGenerator("LeagueOfLegendsClient", Enums);
    ...
    protected override EndpointDefinition? GetMethodObjectToEndpointDefinition(LcuMethodObject getMethodObject,
        string path, LcuPathObject pathObject)
    {
        ...
        var baseMethod = Enums.Contains(returnType)
            ? "GetEnumAsync"
            : GetRiotHttpClientMethod(HttpMethod.Get, returnType, ref typeArgument);
        ...
        return new EndpointDefinition("Get" + nameFromPath, returnType, "HttpClient", baseMethod, typeArgument, path,
            null, pathParameters);
    }

    protected override EndpointDefinition? PostMethodObjectToEndpointDefinition(LcuMethodObject postMethodObject,
        string path, LcuPathObject pathObject)
    {
        throw new NotImplementedException();
    }

    protected override EndpointDefinition? PutMethodObjectToEndpointDefinition(LcuMethodObject putMethodObject,
        string path, LcuPathObject pathObject)
    {
        throw new NotImplementedException();
    }
    ...
    {
        var nestedMembers = _moduleProperties.SelectMany(ps => ps).Select(p => p).Concat(_moduleClasses).ToArray();
...
#pragma warning restore CS8714 // The type cannot be used as type parameter in the generic type or method. Nullability of type argument doesn't match 'notnull' constraint.
    ...
    internal class LeagueClientModuleGenerator : LeagueClientEndpointsGenerator
    {
        ...
        public LeagueClientModuleGenerator(string moduleName, string[] enums, bool methodVersionSuffix,
            bool generateConstructor) : base(moduleName + "Client", enums, false) // For now, adding Client suffix.
        {
            ...
            if (generateConstructor)
            {
                ...
                var constructor = InternalConstructorDeclaration(ClassName, LEAGUECLIENTBASE_CLASS_IDENTIFIER,
                    "leagueClient",
            ...
            var fieldName = "_" + moduleName.ToCamelCase();
            _fieldDeclaration = PrivateFieldDeclaration(ClassName, fieldName);
            _propertyDeclaration =
                FieldBackedPublicReadOnlyPropertyDeclaration(ClassName, ModuleName, fieldName, "this");
        }

        public MemberDeclarationSyntax[] FieldAndProperty => new MemberDeclarationSyntax[]
            {_fieldDeclaration, _propertyDeclaration};
        ...
        public override void AddGroupsAsNestedClassesWithEndpoints(IEnumerable<IGrouping<string?, Path>> groupedPaths,
            IEnumerable<IGrouping<string, KeyValuePair<string, string>>> groupedEvents, string? className = null)
        {
            throw new NotImplementedException();
        }
        ...
        internal void AddPathsAsEndpoints(IGrouping<string?, Path> group,
            IGrouping<string, KeyValuePair<string, string>>? eventGroup)
        {
