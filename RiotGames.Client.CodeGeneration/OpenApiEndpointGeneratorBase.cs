...
internal abstract class OpenApiEndpointGeneratorBase<TPathObject, TGetMethodObject, TPostMethodObject, TPutMethodObject,
    TParameter, TSchema>
    ...
    where TSchema : OpenApiSchemaObject
{
    ...
    protected virtual void AddEndpoint(EndpointDefinition? endpoint)
    {
        ...
        endpoint.RequestUri =
            endpoint.RequestUri.ReplaceStart("$\"/", "$\"").ReplaceStart("\"/", "\""); // Make relative

        var bodyStatement = endpoint.RequestValueType == null
            ? CancellableReturnAwaitStatement(endpoint.HttpClientIdentifier, endpoint.HttpClientMethod,
                endpoint.HttpReturnType, endpoint.RequestUri)
            : CancellableReturnAwaitStatement(endpoint.HttpClientIdentifier, endpoint.HttpClientMethod,
                endpoint.HttpReturnType, endpoint.RequestUri, "value");

        Class = Class.AddCancellablePublicAsyncTask(endpoint.ReturnTypeName, endpoint.Identifier.EndWith("Async"),
            bodyStatement, endpoint.RequestPathParameters);
    }
    ...
    protected abstract EndpointDefinition? GetMethodObjectToEndpointDefinition(TGetMethodObject getMethodObject,
        string path, TPathObject pathObject);

    protected abstract EndpointDefinition? PostMethodObjectToEndpointDefinition(TPostMethodObject postMethodObject,
        string path, TPathObject pathObject);

    protected abstract EndpointDefinition? PutMethodObjectToEndpointDefinition(TPutMethodObject putMethodObject,
        string path, TPathObject pathObject);
    ...
    protected class EndpointDefinition
    {
        ...
        public Dictionary<string, string>? RequestPathParameters;
        public string RequestUri;
        public string? RequestValueType;
        public string ReturnTypeName;

        public EndpointDefinition(string identifier, string returnTypeName, string? httpClientIdentifier,
            string httpClientMethod, string httpReturnType, string requestUri, string? requestValueType,
            Dictionary<string, string>? requestPathParameters)
        {
