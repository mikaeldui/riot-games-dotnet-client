    ...
    where TObjectBase : IRiotGamesObject
{
    ...
    internal RiotGamesClientBase(string apiKey, PlatformRoute platform, RegionalRoute? region = null,
        bool createRegionalClient = true)
    {
        Platform = platform;
    ...
    internal RiotGamesApiHttpClient<TObjectBase> RegionalClient =>
        _regionalClient ?? throw new RiotGamesRouteException("region");

    internal RiotGamesApiHttpClient<TObjectBase> PlatformClient =>
        _platformClient ?? throw new RiotGamesRouteException("platform");
